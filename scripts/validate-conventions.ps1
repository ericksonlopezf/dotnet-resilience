...
<#
.SYNOPSIS
    Automated Convention and Quality Gate Validator for EricksonLopez.Resilience.
.DESCRIPTION
...
foreach ($file in $textFiles) {
    ...
    if ($content -match 'github\.com/ericksonlopez/dotnet-resilience') {
        $rel = Resolve-Path -Relative -Path $file.FullName
        $violations.Add("Legacy GitHub repository URL found in: $rel (expected: github.com/ericksonlopezf/dotnet-resilience)")
        $legacyUrlCount++
    }
