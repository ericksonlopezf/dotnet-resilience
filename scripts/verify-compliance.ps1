...
<#
.SYNOPSIS
    Architecture & Quality Standards Compliance Verification Script for EricksonLopez.Resilience.
.DESCRIPTION
    ...
    5. Valid GitHub repository links referencing ericksonlopezf/dotnet-resilience.
...
Write-Host "  Repository: EricksonLopez.Resilience            " -ForegroundColor Cyan
Write-Host "==================================================" -ForegroundColor Cyan
...
Write-Host "`n[5/7] Checking GitHub identity links (ericksonlopezf/dotnet-resilience)..." -ForegroundColor Yellow
$badLinks = 0
...
foreach ($f in $allTrackedFiles) {
    ...
    for ($i = 0; $i -lt $lines.Count; $i++) {
        if ($lines[$i] -match "github\.com/ericksonlopez/dotnet-resilience\b") {
...
if ($badLinks -eq 0) { Write-Host "  ✅ All GitHub URLs correctly target ericksonlopezf/dotnet-resilience." -ForegroundColor Green }
...
# -----------------------------------------------------------------------------
# Changelog Canonical Structure & Quadruple Synchrony Gate
# -----------------------------------------------------------------------------
Write-Host "`n[Gate: Changelog & Version Synchrony] Validating canonical changelog and version synchrony..." -ForegroundColor Yellow
$changelogErrors = 0
...
if (-not (Test-Path $changelogPath)) {
    ...
    if (-not $latestVersion) {
        Write-Host "  ❌ First version below [Unreleased] does not match canonical format '## [X.Y.Z] - AAAA-MM-DD' with 10-digit ISO date." -ForegroundColor Red
        $violations++
        $changelogErrors++
    } else {
        # Check Directory.Build.props VersionPrefix parity
        $bldPropsPath = Join-Path $targetRoot "Directory.Build.props"
        if (Test-Path $bldPropsPath) {
            $bldPropsXml = [xml](Get-Content $bldPropsPath -Raw)
            $versionPrefix = $bldPropsXml.Project.PropertyGroup.VersionPrefix | Where-Object { $_ } | Select-Object -First 1
            if ($versionPrefix -ne $latestVersion) {
                Write-Host "  ❌ Version mismatch: Directory.Build.props VersionPrefix ('$versionPrefix') does not match CHANGELOG.md version ('$latestVersion')." -ForegroundColor Red
                $violations++
                $changelogErrors++
            } else {
                Write-Host "  [INFO] VersionPrefix parity verified: '$versionPrefix' matches CHANGELOG.md." -ForegroundColor Gray
            }
        }

        # Inspect .release-please-manifest.json (Release Please manages its own release lifecycle)
        $manifestPath = Join-Path $targetRoot ".release-please-manifest.json"
        if (Test-Path $manifestPath) {
            $manifestJson = Get-Content $manifestPath -Raw | ConvertFrom-Json
            $manifestVersion = $manifestJson.'.'
            Write-Host "  [INFO] .release-please-manifest.json detected: '$manifestVersion' (managed by Release Please)." -ForegroundColor Gray
        }
    }
}

if ($changelogErrors -eq 0) {
    Write-Host "  ✅ Canonical changelog structure and version synchrony verified." -ForegroundColor Green
}

# -----------------------------------------------------------------------------
# NuGet Zero-Trust Configuration & Strict PackageProjectUrl Gate
# -----------------------------------------------------------------------------
Write-Host "`n[Gate: NuGet & Local Packages] Validating nuget.config zero-trust, no local packages, and PackageProjectUrl..." -ForegroundColor Yellow
$nugetGateErrors = 0

# 1. Prohibit ./local-packages folder
$localPackagesPath = Join-Path $targetRoot "local-packages"
if (Test-Path $localPackagesPath) {
    Write-Host "  ❌ Prohibited directory ./local-packages detected. Local package feeds are forbidden." -ForegroundColor Red
    $violations++
    $nugetGateErrors++
}

# 2. Enforce root nuget.config
$nugetConfigPath = Join-Path $targetRoot "nuget.config"
if (-not (Test-Path $nugetConfigPath)) {
    Write-Host "  ❌ Missing root nuget.config. Repository must explicitly declare trusted package sources." -ForegroundColor Red
    $violations++
    $nugetGateErrors++
} else {
    $nugetXml = [xml](Get-Content $nugetConfigPath -Raw)
    $sources = $nugetXml.configuration.packageSources.add
    $hasClear = $nugetXml.configuration.packageSources.clear -ne $null
    if (-not $hasClear) {
        Write-Host "  ❌ nuget.config must include <clear /> to isolate sources from machine-level fallback." -ForegroundColor Red
        $violations++
        $nugetGateErrors++
    }
    foreach ($src in $sources) {
        if ($src.value -match '^\.\/' -or $src.value -match 'local-packages' -or $src.value -notmatch '^https:\/\/') {
            Write-Host "  ❌ nuget.config contains prohibited local/insecure package source: key='$($src.key)', value='$($src.value)'." -ForegroundColor Red
            $violations++
            $nugetGateErrors++
        }
    }
...
# 3. Strict PackageProjectUrl validation (No dotnet- prefix)
$bldPropsPath = Join-Path $targetRoot "Directory.Build.props"
if (Test-Path $bldPropsPath) {
    $bldPropsXml = [xml](Get-Content $bldPropsPath -Raw)
    $projectUrl = $bldPropsXml.Project.PropertyGroup.PackageProjectUrl | Where-Object { $_ } | Select-Object -First 1
    if (-not $projectUrl -or $projectUrl -notmatch '^https:\/\/ericksonlopez\.dev\/[a-z0-9-]+$') {
        Write-Host "  ❌ Invalid PackageProjectUrl '$projectUrl'. Must match 'https://ericksonlopez.dev/{lib}'." -ForegroundColor Red
        $violations++
        $nugetGateErrors++
    }
    if ($projectUrl -match 'dotnet-') {
        Write-Host "  ❌ PackageProjectUrl '$projectUrl' contains prohibited 'dotnet-' prefix." -ForegroundColor Red
        $violations++
        $nugetGateErrors++
    }
}

if ($nugetGateErrors -eq 0) {
    Write-Host "  ✅ NuGet zero-trust sources, no local packages, and PackageProjectUrl verified." -ForegroundColor Green
}

# -----------------------------------------------------------------------------
# Implicit Usings Strict Disable Gate (Principle 2)
# -----------------------------------------------------------------------------
Write-Host "`n[Gate: Implicit Usings] Validating <ImplicitUsings>disable</ImplicitUsings> across all projects..." -ForegroundColor Yellow
$implicitUsingsErrors = 0

$allCsprojFiles = Get-ChildItem -Path $targetRoot -Recurse -Filter "*.csproj" -File -ErrorAction SilentlyContinue | Where-Object {
    $_.FullName -notmatch '[\\/](bin|obj|\.git|scratch)[\\/]'
}

foreach ($proj in $allCsprojFiles) {
    $projRaw = Get-Content $proj.FullName -Raw
    if ($projRaw -match '<ImplicitUsings>\s*enable\s*</ImplicitUsings>') {
        Write-Host "  ❌ Prohibited <ImplicitUsings>enable</ImplicitUsings> found in: $($proj.FullName)" -ForegroundColor Red
        $violations++
        $implicitUsingsErrors++
    }
}

# Also verify Directory.Build.props centralizes disable
if (Test-Path $bldPropsPath) {
    $bldRaw = Get-Content $bldPropsPath -Raw
    if ($bldRaw -notmatch '<ImplicitUsings>\s*disable\s*</ImplicitUsings>') {
        Write-Host "  ❌ Directory.Build.props must centralize '<ImplicitUsings>disable</ImplicitUsings>'." -ForegroundColor Red
        $violations++
        $implicitUsingsErrors++
    }
}
...
if ($implicitUsingsErrors -eq 0) {
    Write-Host "  ✅ Zero tolerance for ImplicitUsings=enable verified across all $($allCsprojFiles.Count) projects." -ForegroundColor Green
}

# -----------------------------------------------------------------------------
# Prohibited Compiler Warning Suppressions Gate (CS0618, CS0619, CS1591)
# -----------------------------------------------------------------------------
Write-Host "`n[Gate: Prohibited NoWarn] Validating zero suppressions for CS0618, CS0619, and CS1591..." -ForegroundColor Yellow
$noWarnErrors = 0
$projectAndProps = Get-ChildItem -Path $targetRoot -Recurse -Include "*.csproj", "*.props", "*.targets" -File -ErrorAction SilentlyContinue | Where-Object {
    $_.FullName -notmatch '[\\/](bin|obj|\.git|scratch)[\\/]'
}

foreach ($item in $projectAndProps) {
    $content = Get-Content $item.FullName -Raw
    if ($content -match '<NoWarn>[^<]*\b(CS0618|CS0619|CS1591)\b') {
        Write-Host "  ❌ Prohibited <NoWarn> suppression detected in $($item.FullName): $($matches[0])" -ForegroundColor Red
        $violations++
        $noWarnErrors++
    }
}

if ($noWarnErrors -eq 0) {
    Write-Host "  ✅ Zero prohibited compiler warning suppressions (CS0618, CS0619, CS1591) verified." -ForegroundColor Green
}
