...
4. **Configuration Binding**: `ResilienceConfigurationExtensions` uses explicit scalar parsers (`int.TryParse`, `double.TryParse`, `TimeSpan.TryParse`) and switch expressions for enum resolution (`BackoffType`, `RateLimiterType`) — no `Enum.TryParse<T>` reflection — to bind options from `IConfigurationSection` (ADR-009).
